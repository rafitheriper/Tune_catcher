...
# Edit the content below to easily change the 'About' tab information.
ABOUT_INFO = {
    "app_name": "TuneCatcher",
    "version": "1.9",
    "tagline": "A modern downloader for video and audio.",
    "description": (
        "This application allows you to download video and audio from a wide variety of websites. "
        "It is built using modern Python tools and is designed to be simple and intuitive.\n\n"
        "Simply paste a URL into the downloader, select your desired format, and click download."
    ),
    "credits": (
        "Built with:\n"
        "• Python\n"
...
class TuneCatcher(ctk.CTk):
    ...
    def create_widgets(self):
        ...
        about_frame = ctk.CTkScrollableFrame(about_tab); about_frame.grid(row=0, column=0, padx=15, pady=15, sticky="nsew")
        about_frame.grid_columnconfigure(0, weight=1)

        # Title & Version
        title_label = ctk.CTkLabel(about_frame, text=f"{ABOUT_INFO['app_name']} v{ABOUT_INFO['version']}", font=ctk.CTkFont(size=24, weight="bold"))
        title_label.pack(pady=(10, 5), padx=10, anchor="w")

        # Tagline
        tagline_label = ctk.CTkLabel(about_frame, text=ABOUT_INFO['tagline'], font=ctk.CTkFont(size=14, slant="italic"))
        tagline_label.pack(pady=(0, 20), padx=10, anchor="w")

        # Description
        desc_label = ctk.CTkLabel(about_frame, text=ABOUT_INFO['description'], wraplength=600, justify="left")
        desc_label.pack(pady=10, padx=10, anchor="w", fill="x")

        # Credits
        credits_label = ctk.CTkLabel(about_frame, text=ABOUT_INFO['credits'], wraplength=600, justify="left")
        credits_label.pack(pady=10, padx=10, anchor="w", fill="x")
